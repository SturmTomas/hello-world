...
public class PaymentController {
	...
	private void updateTableView() throws HashMapNotFoundException {
		...
		ucetLabel.setText("Zostatok na účte: "+ su.getUcet().toString()+ " EUR");
			...
			for (Animal animal : valueList) {
				if (!(animal instanceof Dog) && !(animal instanceof Cat)) {
					...
					if (animal instanceof Dog) {
						...
						Button payButton = new Button("Zaplatiť");
	...
	private void payForAnimal(String animalName)  {
		...
		if( animal instanceof Dog){
			if(su.getUcet() >= ((Dog)animal).getPayment()){
				...
				nedostatokLabel.setText("Nedostatok peňazí na účte!");
				Alert alert = new Alert(Alert.AlertType.ERROR);
				alert.setTitle("Error");
				alert.setHeaderText("Transakcia neprebehla!");
				alert.setContentText("Nedostatok peňazí na účte!");
				alert.showAndWait();
			}
		}
		if( animal instanceof Cat){
	...
	public void requestMoney(ActionEvent actionEvent) {
		...
		try {
			...
			alert.setHeaderText("Nesprávny formát sumy");
			alert.setContentText("Zadajte sumu ako číslo");
			alert.showAndWait();
			return;
		}
		...
		if(su.getWantMoney() == null) {
			...
			alert.setTitle("Info");
			alert.setHeaderText("Žiadosť bola zaslaná!");
			alert.setContentText("Počkajte na schválenie žiadosti administrátorom");
			alert.showAndWait();
		}else{
			Alert alert = new Alert(Alert.AlertType.WARNING);
			alert.setTitle("Pozor");
			alert.setHeaderText("Predchádzajúca Žiadosť sa ešte vybavuje!");
			alert.setContentText("Počkajte na vybavenie žiadosti administrátorom");
			alert.showAndWait();
		}
	}
}
